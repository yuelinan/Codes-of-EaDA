...
import random
## training
from model import GraphCell
from utils import init_weights, get_args, eval_test,train_fed_baseline,eval_test_class, train_cell,get_kmeans
import copy
...
logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                    ...
                    filename='/data/lnyue/GREA-main/logger_fed/'+ 'base-fed-' +str(args_first.date)+'_'+args_first.model_name+str(args_first.batch_size) +args_first.dataset+'_'+args_first.gnn+'_'+'algin_loss_'+str(args_first.algin_loss)+ '.log',
                    filemode='w'
                    )
    ...
    for epoch in range(args.epochs):
        ...
        for client_idx, model in enumerate(models):
            for i in range(10):
                ...
                train_cell(args, model, device, train_loader, optimizers, dataset.task_type, optimizer_name,loss_logger,communication_env_all,server_model)
        ...
        # for index_client, client_model in enumerate(bias_models):
        #     client_model.eval()
        #     client_test_auc = eval_test(args, client_model, device, test_loader, evaluator)[0]
        #     valid_test_auc =  eval_test(args, client_model, device, valid_loader, evaluator)[0]
        #     client_test_all[index_client].append(client_test_auc)
        #     client_valid_all[index_client].append(valid_test_auc)
        #     client_model.train()
        ...
        if update_test or epoch == 0:
            ...
            #     class_test_auc1 = class_test_perfs[0]
            #     logger.info("=====Epoch {}, Metric: {}, Validation: {}, Test: {}, Class_Test:{}".format(epoch, 'AUC', valid_perf, test_auc1,class_test_auc1))
            ...
            if cnt_wait > args.patience:
                break
        PATH = '/data/lnyue/GREA-main/spmotif_codes/car_output/'+'_hiv_'+str(epoch)
